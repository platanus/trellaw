...
describe('actions', () => {
  describe('getBoard', () => {
    it('sends GET_BOARD_SUCCESS on success', (done) => {
      testHelpers.testAction(actions.getBoard, '1', {}, [
        { type: types.GET_BOARD },
        { type: types.GET_BOARD_SUCCESS, payload: boardResponse.data },
      ], done);
    });

    it('sends GET_BOARD_FAIL on failure', (done) => {
      testHelpers.testAction(actions.getBoard, '2', {}, [
        { type: types.GET_BOARD },
        { type: types.GET_BOARD_FAIL },
      ], done);
    });
  });
});
